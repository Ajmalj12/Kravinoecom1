...
const MiddleBanner = () => {
  ...
  const fetchBanners = async () => {
    try {
      const response = await fetch(`${backendUrl}/api/banner/list?section=middle`);
      const data = await response.json();
      
      console.log('Middle Banner API Response:', data);
      ...
      if (data.success && data.banners.length > 0) {
        ...
        console.log('Transformed middle banners:', transformedBanners);
        setBanners(transformedBanners);
      } else {
        console.log('No middle banners found');
        setBanners([]);
      }
      setLoading(false);
    } catch (error) {
